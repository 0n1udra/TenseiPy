...
def action_menu(current_class):
    ...
    if 'stats' in user_input:
        rimuru.show_attributes(split_user_input)
    elif 'info' in user_input:
        rimuru.show_info(split_user_input)
    elif 'use' in user_input:
        skill_success = rimuru.use_skill(split_user_input)
    elif 'attack' in user_input:
        attacked, attack_success = rimuru.attack(split_user_input)
    try:
        pass
    except:
        pass
...
def add_level_mob(level_characters):
    ...
    if type(level_characters) == list:
        ...
        mob = rimuru.get_object(level_characters, new=True)
        rimuru.current_level_characters.append(rimuru.get_object(mob, new=True))
