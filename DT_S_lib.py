...
# pauses game
def wait():
    input(" :") # pauses game, until press 'enter'
...
def game_Over(): # if you die, this asks you if you want to play again
    ...
    try: # this is if somebody inputs something that is not usable it goes to except, instead of crasing
        if inp in ('y', 'yes'):
            Startup() # if inputted 'y' or 'yes' it goes back to main menu
...
def error():
    input("That didn't work : ") # simple error message

# makes sure this program doesn't run, if runned directly
if __name__ == '__main__':
    print("Shuld not be runned directly!")
else:
    print("Drake's Standard Library... Imported.")